-- 096 — إصلاح خصومات الرواتب: أيام عمل فقط، حد التأخير، عدم تكرار اليوم
...
BEGIN
  ...
  END IF;
  WHILE d <= p_end LOOP
    -- 5=جمعة، 6=سبت (عطلة نهاية الأسبوع الشائعة)
...
CREATE OR REPLACE FUNCTION saas_v3_billable_late_minutes(
...
AS $$
BEGIN
  -- Deduct exact late minutes, no threshold gate.
  RETURN GREATEST(0, p_late_minutes);
END;
$$;
  ...
  LOOP
    ...
    IF att.check_in IS NOT NULL
      ...
      IF NOT is_comm
        ...
        -- Compute late directly from check-in delta to avoid stale/incorrect historical late text.
        ...
        billable_late_min := saas_v3_billable_late_minutes(emp.company_id, computed_late_min);
        total_late_min := total_late_min + billable_late_min;
      END IF;
...
REVOKE ALL ON FUNCTION saas_v3_billable_late_minutes(INTEGER, INTEGER) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION saas_v3_billable_late_minutes(INTEGER, INTEGER) TO authenticated;
