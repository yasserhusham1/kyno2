-- 092 — Fix late minutes parsing + open-hours absence counting
...
BEGIN
  ...
  -- Support HH:MM (e.g. 01:20 -> 80 minutes)
  IF s ~ '^\d{1,2}:\d{2}$' THEN
    hh := split_part(s, ':', 1);
    mm := split_part(s, ':', 2);
    RETURN GREATEST(0, COALESCE(hh::INTEGER, 0) * 60 + COALESCE(mm::INTEGER, 0));
  END IF;

  -- Support "1س 20د" or "1h 20m"
  x := regexp_match(s, '(\d+)\s*(?:س|h|hour)', 'i');
  IF x IS NOT NULL AND array_length(x, 1) >= 1 THEN
    h := COALESCE(x[1]::INTEGER, 0);
  END IF;
  x := regexp_match(s, '(\d+)\s*(?:د|m|min)', 'i');
  IF x IS NOT NULL AND array_length(x, 1) >= 1 THEN
    m := COALESCE(x[1]::INTEGER, 0);
  END IF;
  IF h > 0 OR m > 0 THEN
    RETURN GREATEST(0, (h * 60) + m);
  END IF;

  -- Fallback plain numbers => minutes
  hh := NULLIF(regexp_replace(s, '[^0-9]', '', 'g'), '');
  RETURN GREATEST(0, COALESCE(hh::INTEGER, 0));
END;
$$;
...
BEGIN
  ...
  -- Open-hours employees should also auto-count missing days as absent.
  IF is_comm THEN
    expected_absent := 0;
  ELSE
  ...
  IF is_comm THEN
    ...
    ot_amount := 0;
  ELSE
    late_deduct := CASE WHEN emp.open_hours IS TRUE THEN 0 ELSE ROUND(total_late_min * late_deduct_rate) END;
