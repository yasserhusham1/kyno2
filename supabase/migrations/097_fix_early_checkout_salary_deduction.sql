-- 097 — خصم نقص الدوام من بطاقة الموظف
-- يحسب الخصم من: تأخير الدخول + الانصراف المبكر، ولا يعتمد على قيم late القديمة.

CREATE OR REPLACE FUNCTION saas_v3_attendance_short_minutes(
  p_company_id INTEGER,
  p_official_check_in TIME,
  p_official_check_out TIME,
  p_actual_check_in TEXT,
  p_actual_check_out TEXT
)
RETURNS INTEGER
LANGUAGE plpgsql
STABLE
...
DECLARE
  ws TEXT;
  we TEXT;
  official_ci INTEGER := 0;
  official_co INTEGER := 0;
  actual_ci INTEGER := 0;
  actual_co INTEGER := 0;
  late_in INTEGER := 0;
  early_out INTEGER := 0;
BEGIN
  IF p_actual_check_in IS NULL OR p_actual_check_in = '—'
     OR p_actual_check_out IS NULL OR p_actual_check_out = '—' THEN
    RETURN 0;
  END IF;

  ws := NULLIF(saas_v3_company_setting(p_company_id, 'work_start'), '');
  we := NULLIF(saas_v3_company_setting(p_company_id, 'work_end'), '');

  official_ci := basma_db_time_to_minutes(
    COALESCE(p_official_check_in, NULLIF(ws, '')::TIME, TIME '08:00')
  );
  official_co := basma_db_time_to_minutes(
    COALESCE(p_official_check_out, NULLIF(we, '')::TIME, TIME '17:00')
  );
  actual_ci := basma_time_text_to_minutes(p_actual_check_in);
  actual_co := basma_time_text_to_minutes(p_actual_check_out);

  late_in := GREATEST(0, actual_ci - official_ci);
  early_out := GREATEST(0, official_co - actual_co);
  RETURN GREATEST(0, late_in + early_out);
END;
$$;
...
BEGIN
  ...
  period_elapsed_end := LEAST(basma_date_iso_baghdad(), bounds.period_end);
  elapsed_work_days := saas_v3_work_days_between(bounds.period_start, period_elapsed_end);
...
REVOKE ALL ON FUNCTION saas_v3_attendance_short_minutes(INTEGER, TIME, TIME, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION saas_v3_attendance_short_minutes(INTEGER, TIME, TIME, TEXT, TEXT) TO authenticated;
