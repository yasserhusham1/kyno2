-- 118 — persist admin_reason on attendance (hire backfill vs admin batch)
...
DECLARE
  ...
  use_reason TEXT := NULLIF(trim(p_reason), '');
  ci_min INTEGER;
  co_min INTEGER;
  official_ci INTEGER;
  official_co INTEGER;
...
BEGIN
  ...
  SELECT a.check_in, a.check_out, a.status, a.late, a.admin_reason INTO existing
  FROM attendance a
  WHERE a.employee_id = p_employee_id AND a.date_iso = p_date_iso
  LIMIT 1;
  ...
  ON CONFLICT (employee_id, date_iso) DO UPDATE SET
    ...
    admin_reason = COALESCE(EXCLUDED.admin_reason, attendance.admin_reason)
  ...
  PERFORM saas_v3_write_audit(
    ...
    COALESCE(use_reason, 'Admin attendance upsert'),
    emp.name,
    emp.company_id,
    before_row,
    after_row
