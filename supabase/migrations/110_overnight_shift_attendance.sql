-- 110 — دعم الدوام الليلي (مثلاً 22:00 → 05:00): انصراف بعد منتصف الليل على سجل حضور اليوم السابق

CREATE OR REPLACE FUNCTION saas_v3_is_overnight_shift(
  p_check_in TIME,
  p_check_out TIME
)
RETURNS BOOLEAN
...
IMMUTABLE
AS $$
  SELECT basma_db_time_to_minutes(COALESCE(p_check_out, TIME '17:00'))
      <= basma_db_time_to_minutes(COALESCE(p_check_in, TIME '08:00'));
$$;

CREATE OR REPLACE FUNCTION saas_v3_employee_work_minutes(
  p_check_in TIME,
  p_check_out TIME,
...
BEGIN
  ...
  ci_min := basma_db_time_to_minutes(
    COALESCE(p_check_in, NULLIF(ws, '')::TIME, TIME '08:00')
  );
  co_min := basma_db_time_to_minutes(
    COALESCE(p_check_out, NULLIF(we, '')::TIME, TIME '17:00')
  );

  IF co_min <= ci_min THEN
    RETURN GREATEST(60, (24 * 60 - ci_min) + co_min);
  END IF;
  RETURN GREATEST(60, co_min - ci_min);
END;
$$;
