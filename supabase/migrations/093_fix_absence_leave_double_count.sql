-- 093 — Fix absent double-count: exclude all approved leave days from auto-absence
...
BEGIN
  ...
  LOOP
    covered := covered + saas_v3_leave_days_in_period(
      lv.from_date,
      lv.to_date,
      p_period_start,
      p_period_end
...
BEGIN
  ...
  LOOP
    ...
    IF lbl IN ('paid_single', 'paid_open') THEN
      paid_days := paid_days + saas_v3_leave_days_in_period(
        lv.from_date,
        lv.to_date,
        p_period_start,
        p_period_end
...
BEGIN
  ...
  -- Auto-absence only for days without attendance, without explicit غياب, and without approved leave.
  IF is_comm THEN
    expected_absent := 0;
  ELSE
